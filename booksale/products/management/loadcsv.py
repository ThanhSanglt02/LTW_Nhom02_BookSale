...
from products.models import Publisher, Contributor, Book, BookContributor, Review


class Command(BaseCommand):
    help = 'Load the reviews data from a CSV file.'

    # Django sẽ chuyển phần --csv reviews/... xuống cho class Command
    # def này chạy trước
    def add_arguments(self, parser):
        parser.add_argument('--csv', type=str)
    # --> Có một **option có key csv với giá trị đi kèm là string (ở đaya là đường dẫn từ reviews truy cập đến file csv WebDevWith...csv)
    ...
    # **options = {'csv': 'reviews/management/commands/WebDevWithDjangoData.csv'}
    # sau khi def add_argument chạy xong thì truyền option vào handle, handle tiến hành chạy và xử lý
